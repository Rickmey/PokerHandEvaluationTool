namespace PokerHandEvaluator.TexasHoldem
{
    ...
    internal static class EvaluatorOriginal
    {
        ...
        public static uint GetHandValue(ulong cards, int nCards)
        {
            ...
            uint n_ranks = Utils.nBitsTable[ranks];
            ...
            */
            if (n_ranks >= 5)
            {
                if (Utils.nBitsTable[ss] >= 5)
                {
                    if (LookupTables.straightTable[ss] != 0)
                        return HANDTYPE_VALUE_STRAIGHTFLUSH + (uint)(LookupTables.straightTable[ss] << TOP_CARD_SHIFT);
                    else
                        retval = HANDTYPE_VALUE_FLUSH + topFiveCardsTable[ss];
                }
                else if (Utils.nBitsTable[sc] >= 5)
                {
                    if (LookupTables.straightTable[sc] != 0)
                        return HANDTYPE_VALUE_STRAIGHTFLUSH + (uint)(LookupTables.straightTable[sc] << TOP_CARD_SHIFT);
                    else
                        retval = HANDTYPE_VALUE_FLUSH + topFiveCardsTable[sc];
                }
                else if (Utils.nBitsTable[sd] >= 5)
                {
                    if (LookupTables.straightTable[sd] != 0)
                        return HANDTYPE_VALUE_STRAIGHTFLUSH + (uint)(LookupTables.straightTable[sd] << TOP_CARD_SHIFT);
                    else
                        retval = HANDTYPE_VALUE_FLUSH + topFiveCardsTable[sd];
                }
                else if (Utils.nBitsTable[sh] >= 5)
                {
                    if (LookupTables.straightTable[sh] != 0)
                        return HANDTYPE_VALUE_STRAIGHTFLUSH + (uint)(LookupTables.straightTable[sh] << TOP_CARD_SHIFT);
                    else
            ...
            switch (n_dups)
            {
                ...
                default:
                    ...
                    two_mask = ranks ^ (sc ^ sd ^ sh ^ ss);
                    if (Utils.nBitsTable[two_mask] != n_dups)
                    {
